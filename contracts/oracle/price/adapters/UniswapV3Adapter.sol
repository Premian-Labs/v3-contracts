...
contract UniswapV3Adapter is AccessControl, SimpleOracle, IUniswapV3Adapter {
    ...
    function canSupportPair(
        ...
        try UNISWAP_V3_ORACLE.getAllPoolsForPair(_tokenA, _tokenB) returns (
    ...
    function _addOrModifySupportForPair(
        ...
        uint16 _targetCardinality = uint16(
            (period * cardinalityPerMinute) / 60
        ) + 1;
        uint256 _preparedPools;
        for (uint256 i; i < _pools.length; i++) {
            address _pool = _pools[i];
            (, , , , uint16 _currentCardinality, , ) = IUniswapV3Pool(_pool)
                .slot0();
            if (_currentCardinality < _targetCardinality) {
                uint112 _gasCostToIncreaseAndAddSupport = uint112(
                    _targetCardinality - _currentCardinality
                ) *
        ...
        if (_pools.length > 1) {
            ...
            for (uint256 i; i < _pools.length - 1; i++) {
                ...
                for (uint256 j = i + 1; j < _pools.length; j++) {
                    if (
                        _poolLiquidity[j] >
                        _poolLiquidity[_biggestLiquidityIndex]
                    ) {
                ...
                if (_biggestLiquidityIndex != i) {
                    // Swap pools
                    (_pools[i], _pools[_biggestLiquidityIndex]) = (
                        _pools[_biggestLiquidityIndex],
                        _pools[i]
                    );
