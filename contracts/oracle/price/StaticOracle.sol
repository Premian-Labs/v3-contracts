...
import {IStaticOracle, IUniswapV3Factory} from "./IStaticOracle.sol";

/// @title Uniswap V3 Static Oracle
/// @notice Oracle contract for price quoting against Uniswap V3 pools
/// @notice derived from https://github.com/Mean-Finance/uniswap-v3-oracle
contract StaticOracle is IStaticOracle {
    ...
    function _getCardinalityForTimePeriod(
        uint32 _period
    ) internal view returns (uint16 _cardinality) {
    ...
    function _quote(
        ...
        OracleLibrary.WeightedTickData[]
            memory _tickData = new OracleLibrary.WeightedTickData[](
                _pools.length
            );
        for (uint256 i; i < _pools.length; i++) {
