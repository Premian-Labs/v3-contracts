...
contract PoolCore is IPoolCore, PoolInternal, ReentrancyGuard {
    ...
    function exerciseFor(
        ...
        for (uint256 i = 0; i < holders.length; i++) {
            if (holders[i] != msg.sender) {
                _revertIfOperatorNotAuthorized(holders[i], IUserSettings.Action.EXERCISE);
    ...
    function settleFor(
        ...
        for (uint256 i = 0; i < holders.length; i++) {
            if (holders[i] != msg.sender) {
                _revertIfOperatorNotAuthorized(holders[i], IUserSettings.Action.SETTLE);
                _revertIfCostNotAuthorized(holders[i], _cost);
            }
    ...
    function settlePositionFor(
        ...
        for (uint256 i = 0; i < p.length; i++) {
            if (p[i].operator != msg.sender) {
                _revertIfOperatorNotAuthorized(p[i].operator, IUserSettings.Action.SETTLE_POSITION);
