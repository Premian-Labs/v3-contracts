...
interface IUserSettings {
    /// @notice Returns true if the agent is authorized to call `exerciseFor`, `settleFor`, or `settlePositionFor` on behalf of the user
    ...
    /// @notice Returns the addresses of agents authorized to call `exerciseFor`, `settleFor`, or `settlePositionFor` on behalf of the user
    ...
    /// @notice Sets the addresses authorized to call `exerciseFor`, `settleFor`, or `settlePositionFor` on behalf of the user
    /// @param agents The addresses of authorized agents
    function setAuthorizedAgents(address[] memory agents) external;

    /// @notice Returns the users authorized cost in the ERC20 Native token (WETH, WFTM, etc) used in conjunction with `exerciseFor`,
    ...
    /// @notice Sets the users authorized cost in the ERC20 Native token (WETH, WFTM, etc) used in conjunction with `exerciseFor`,
    ///        `settleFor`, and `settlePositionFor`
