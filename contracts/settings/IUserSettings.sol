...
interface IUserSettings {
    /// @notice Returns true if the agent is authorized to call `exerciseFor`, `settleFor`, or `settlePositionFor`
    ...
    /// @notice Sets the addresses of authorized to call `exerciseFor`, `settleFor`, or `settlePositionFor`
    ///         on behalf of the user
    /// @param agents The addresses of authorized agents
    function setAuthorizedAgents(address[] memory agents) external;

    /// @notice Returns the users authorized cost in the ERC20 Native token (WETH, WFTM, etc) used in conjunction
    ///         with `exerciseFor`, `settleFor`, and `settlePositionFor`
    ...
    /// @notice Sets the users authorized cost in the ERC20 Native token (WETH, WFTM, etc) used in conjunction
    ///         with `exerciseFor`, `settleFor`, and `settlePositionFor`
