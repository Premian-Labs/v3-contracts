...
abstract contract PoolWithdrawTest is DeployTest {
    function test_withdraw_750LC() public {
        ...
        uint256 withdrawCollateralValue = scaleDecimalsFrom(contractsToCollateral(withdrawSize * avgPrice));
    ...
    function test_withdraw_CS_Straddle() public {
        ...
        uint256 expectedBalance = isCallTest
            ? scaleDecimalsFrom(ud(0.58359375 ether))
            : scaleDecimalsFrom(ud(0.58359375 ether) * poolKey.strike);
    ...
    function test_withdraw_CSUP_Straddle() public {
        ...
        uint256 expectedBalanceAfterDeposit = isCallTest
            ? scaleDecimalsFrom(ud(0.15 ether))
            : scaleDecimalsFrom(ud(0.15 ether) * poolKey.strike);
        ...
        uint256 expectedBalance = isCallTest
            ? scaleDecimalsFrom(ud(0.62109375 ether))
            : scaleDecimalsFrom(ud(0.62109375 ether) * poolKey.strike);
        assertEq(IERC20(getPoolToken()).balanceOf(users.lp), expectedBalance);
    }
    ...
    function test_withdraw_LC_Straddle() public {
        ...
        uint256 expectedBalanceAfterDeposit = isCallTest
            ? scaleDecimalsFrom(ud(0.85 ether))
            : scaleDecimalsFrom(ud(0.85 ether) * poolKey.strike);
        assertEq(IERC20(getPoolToken()).balanceOf(users.lp), expectedBalanceAfterDeposit);
        vm.warp(block.timestamp + 60);
        ...
        uint256 expectedBalance = isCallTest
            ? scaleDecimalsFrom(ud(0.92734375 ether))
            : scaleDecimalsFrom(ud(0.92734375 ether) * poolKey.strike);
        assertEq(IERC20(getPoolToken()).balanceOf(users.lp), expectedBalance);
    }
